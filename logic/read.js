...
function ruta(valor) {
    ...
    if (valor === 'home') {
        ...
        //
    } else if (valor === 'sliders') {
        //
        window.location.href = "index.php?view=sliders";
        sessionStorage.location = 'sliders';
        //
    }
}
...
//        graficaBarras(datos2, categorias, 'titulo');
        ...
        //
    } else if (sessionStorage.location === 'sliders') {
        //
        cargarSelectEmpresas();
        cargarSelectSectores();
        cargarTablaSliders();
        //
        $('#btnSlidersMenu').addClass('active');
    }
}
    ...
    $.ajax({
        ...
        if (data[0].sql === 1) {
            ...
            for (var i = 0; i < arraySectores.length; i++) {
                //
                if (data[0].idSec === arraySectores[i]['idSec']) {
                    //
                    camposS += '<option value="' + arraySectores[i]['idSec'] + '" selected>' + arraySectores[i]['nombre'] + '</option>';
                } else {
                    //
                    camposS += '<option value="' + arraySectores[i]['idSec'] + '">' + arraySectores[i]['nombre'] + '</option>';
                }
            }
            //
...
function cargarSelectSectores() {
    ...
    $.ajax({
        ...
        if (data[0].sql === 1) {
            ...
            for (var i = 0; i < data.length; i++) {
                //
                campos += '<option value="' + data[i]['idSec'] + '">' + data[i]['nombre'] + '</option>';
            }
            //
            $('#sector').html(campos);
        } else {
            //
            $('#sector').html('<option value="" selected disabled>No hay sectores</option>');
        }
    }).fail(function (data_error) {
    });
}
    ...
    if ($('#fechaInicialGrafica').val() == "" || $('#fechaFinalGrafica').val() == "") {
        ...
        $.ajax({
            ...
            var myChartP = Highcharts.chart('containerGraficaTorta', {
                ...
                subtitle: {
                    text: "Cantidad de entradas por sedes"
                },
                tooltip: {
...
function cargarDatosGraficaBarras() {
    //
    ...
    if ($('#fechaInicialGrafica').val() == "" || $('#fechaFinalGrafica').val() == "") {
        ...
        $.ajax({
            ...
            type: 'post',
            data: {
                "accion": "cargarDatosGraficaBarras",
                "fechaInicial": $('#fechaInicialGrafica').val(),
                "fechaFinal": $('#fechaFinalGrafica').val(),
                "idEmp": sessionStorage.idEmp
            },
...
function cargarTablaSliders() {
    //
    var tabla = $('#tablaSliders').DataTable({
        destroy: true,
        ajax: {
            method: "POST",
            url: "controllers/read.php",
        ...
        columns: [
            {
                "render": function (data, type, JsonResultRow, meta) {
                    //
                    if (JsonResultRow.imagen !== '') {
                        //
                        return '<td><img style="width: 3vw;" src="imagenes/sliders/' + JsonResultRow.imagen + '"/></td>';
                    } else {
                        //
                        return '<td><img style="width: 3vw;" src="imagenes/sliders/default.png"/></td>';
                    }
                }
            },
            {
            ...
            },
            {
                "render": function (data, type, JsonResultRow, meta) {
                    ...
                    if (JsonResultRow.estado === 'Activa') {
                        //
                        return '<label class="switch"><input onclick="actualizarEstadoSlider(' + JsonResultRow.idSli + ')" type="checkbox" checked><span class="slider round"></span></label>';
                    } else {
                        //
                        return '<label class="switch"><input onclick="actualizarEstadoSlider(' + JsonResultRow.idSli + ')" type="checkbox"><span class="slider round"></span></label>';
                    }
                }
            },
            {
                "render": function (data, type, JsonResultRow, meta) {
                    return '<button onclick="cargarSlideraEditar(' + JsonResultRow.idSli + ')" type="button" class="btn btn-primary" data-toggle="modal" data-target=".bd-eSlider-modal-xl"><i class="fas fa-pen-square"></i></button>';
                }
            }
        ],
        scrollY: '41vh',
...
//
var idSlideraEditar = 0;
var imagenSliA = '';

//
function cargarSlideraEditar(valor) {
    //
    idSlideraEditar = valor;
    //
    $.ajax({
        url: 'controllers/read.php',
        type: 'post',
        ...
        if (data[0].sql === 1) {
            ...
            //
            swal("Atención", "Error al cargar el slider!");
        }
    }).fail(function (data_error) {
    });
}